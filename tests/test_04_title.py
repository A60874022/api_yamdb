...
from .common import (auth_client, create_Category, create_genre,
...
class Test04TitleAPI:
    ...
    def test_02_title_admin(self, admin_client):
        ...
        data = {'name': 'Поворот', 'year': 2020, 'genre': [Genre[1]['slug']],
                'category': Category[1]['slug'], 'description': 'Крутое пике'}
        admin_client.post('/api/v1/titles/', data=data)
        response = admin_client.get(f'/api/v1/titles/?genre={Genre[1]["slug"]}')
        data = response.json()
        assert len(data['results']) == 2, (
            'Проверьте, что при GET запросе `/api/v1/titles/` фильтуется по `genre` параметру `slug` жанра'
        )
        response = admin_client.get(f'/api/v1/titles/?category={Category[0]["slug"]}')
        data = response.json()
        ...
        invalid_data = {
            'name': 'Поворот', 'year': 'дветыщи шесть', 'genre': [Genre[1]['slug']],
            'category': Category[1]['slug'], 'description': 'Крутое пике'
        }
    ...
    def test_03_titles_detail(self, client, admin_client):
        titles, Category, Genre = create_titles(admin_client)
        ...
        assert data.get('category') == Category[0], (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/` возвращаете данные объекта. '
        ...
        data = response.json()
        assert data.get('category') == Category[1], (
    ...
    def check_permissions(self, user, user_name, titles, Category, Genre):
        client_user = auth_client(user)
        data = {'name': 'Чудо юдо', 'year': 1999, 'genre': [Genre[2]['slug'], Genre[1]['slug']],
                'category': Category[0]['slug'], 'description': 'Бум'}
    ...
    def test_04_titles_check_permission(self, client, admin_client):
        ...
        self.check_permissions(user, 'обычного пользователя', titles, Category, Genre)
        self.check_permissions(moderator, 'модератора', titles, Category, Genre)
